...
export default {
	...
	mounted() {
		...
		this.$nextTick(() => {
			...
			setTimeout(() => {
				this._getCssSafeAreaInsetBottom(() => this.safeAreaInsetBottom && this
	...
	computed: {
		...
		renderJsIgnore() {
			if ((this.usePageScroll && this.useChatRecordMode) || (!this.refresherEnabled && this.scrollable) || !this
				.useCustomRefresher) {
				this.$nextTick(() => {
			...
			try {
				if (shouldFullHeight) {
					...
					if (finalScrollViewNode) {
						...
						const finalHeight = scrollViewHeight + additionHeight - (this.insideMore ? 1 : 0) +
							'px !important';
