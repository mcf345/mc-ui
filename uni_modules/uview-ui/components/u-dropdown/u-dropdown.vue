<template>
	<view class="u-dropdown">
		<view class="u-dropdown__menu" :style="{height: $utils.addUnit(height)}"
			...
			<view class="u-dropdown__menu__item" v-for="(item, index) in menuList" :key="index"
				...
				<view class="x-f" style="flex-direction: row;">{{$utils.addUnit(height)}}
					<text class="u-dropdown__menu__item__text"
						:style="{color: item.disabled ? '#c0c4cc' : (index === current || highlightIndex == index) ? activeColor : inactiveColor,fontSize: $utils.addUnit(titleSize)}">{{item.title}}</text>

					<view class="u-dropdown__menu__item__arrow"
						:class="{'u-dropdown__menu__item__arrow--rotate': index === current}">
			...
			getContentHeight() {
				...
				uni.getSystemInfo({
					success: function(e) {
						...
						that.uGetRect('.u-dropdown__menu').then(res => {
