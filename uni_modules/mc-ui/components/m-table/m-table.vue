<template>
	<view class="m-table-box">
		<view v-if="loading" class="m-table-loading-box" :style="{
		...
		<view v-if="!loading" class="m-table-scroll-view" :style="{
			width: width,
			height: height,
			borderTop: '1px solid' + borderColor,
			borderLeft: showLeftAndRightBorder ? '1px solid' + borderColor : 'none',
			borderRight: showLeftAndRightBorder ? '1px solid' + borderColor : 'none'}">
			<view class="m-table-header" :style="{borderBottom: '1px solid' + borderColor}">
				<view class="m-table-header-item" v-if="enableCheck" :style="{
					 ...
					 backgroundColor: headerBgColor,
					 position: 'sticky'}">
					<view class="m-table-checkbox" v-if="enableCheck === 'multiple'" @tap.stop="onCheckAllTap" :style="{
				...
				<view ref="iosBug" class="m-table-header-item" v-for="(item, index) in headers" :key="item.key"
					...
					<view class="m-table-header-icon" v-if="autoSortShow(index)">
			...
			<view class="m-table-content">
				<view class="m-table-content-line" v-for="(content, cIndex) in contentsSort"
					:key="contentLineKey(content, cIndex)"
					:style="{borderTop: cIndex === 0 ? 'none' : '1px solid' + borderColor}">
					<view class="m-table-content-item" v-if="enableCheck" :style="{
						 minWidth: checkColWidth + 'rpx',
						 maxWidth: checkColWidth + 'rpx',
						 textAlign: textAlign,
						 justifyContent: textAlign === 'center' ? textAlign : (textAlign === 'left' ? 'flex-start' : 'flex-end'),
						 ...
						 left: 0,
						 position: 'sticky'}">
						<view class="m-table-checkbox" @tap.stop="onCheckItemTap(cIndex)" :style="{
								width: checkColWidth * 0.5 + 'rpx',
								height: checkColWidth * 0.5 + 'rpx',
								backgroundColor: checkerBoxBgColor,
								border: '1px solid ' + checkerBorderColor}">
					...
					<view class="m-table-content-item" v-for="(header, hIndex) in headers"
				...
				<view v-if="computedCol.length !== 0" class="m-table-content-line" :style="{
					...
					<view class="m-table-content-item" v-for="(header, index) in headers" :key="index" :style="{
						 minWidth: (header.width || defaultColWidth) + 'rpx',
						 maxWidth: (header.width || defaultColWidth) + 'rpx',
						 textAlign: textAlign,
						 justifyContent: textAlign === 'center' ? textAlign : (textAlign === 'left' ? 'flex-start' : 'flex-end'),
		...
		computed: {
			...
			autoContentItem() {
				return function(cIndex, hIndex) {
					...
					if (header.hasOwnProperty('FormatType')) {
						if (header.FormatType == 'price') {
							return this.$utils.formatPrice(result);
						}
					}
					return result
				}
			...
			autoBottomComputedItem() {
				return function(index) {
					...
					if (header.hasOwnProperty('FormatType')) {
						if (header.FormatType == 'price') {
							return that.$utils.formatPrice(result);
						}
					}
					return result
				}
...
<style>
	...
	.m-table-scroll-view::-webkit-scrollbar {
		display: none;
		/* #ifdef MP-WEIXIN */
		width: 0;
		height: 0;
