let jPreview={
    ...
    startPreviw(){
        ...
        if($.inArray(ext,imgExt)>=0){
            dynamicLoadJs(static+"/common/js/zoom-marker.min.js",function(){
                dynamicLoadJs(static+"/common/js/jquery.mousewheel.min.js",function(){
                    self.imgView(url);
                })
            })
        }else if($.inArray(ext,pdfExt)>=0){
            dynamicLoadJs(static+"/common/js/pdfobject.js",function(){
                self.pdfView(url);
            })
    ...
    pdfView(url){
        new PDFObject({url: url}).embed();
    },
    imgView(url){
        $('#'+this.config.container).addClass("picview").html('<img src="img/none.png" alt="..." name="viewArea" id="viewArea" draggable="false"/>');
        $('#viewArea').zoomMarker({
            src: url,
            rate: 0.2,
        });
